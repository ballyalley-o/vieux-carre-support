...
function FormSelect<T, V>({ name, label, options, value, onChange, getValue, disabled = false, defaultValue, onToggle, renderList }: FormSelectProps<T, V>) {
    const [open, setOpen] = useState(false)
    // const selectedLabel   = value ? getLabel(value) : defaultValue
      ...
      <div className={"relative"}>
       {label &&  <label className={'text-md py-4 mx-2 font-semibold'}>{label}</label>}
        <button type="button" className={'w-full border p-3 input-default flex justify-between items-center'} onClick={() => !disabled && toggleOpen(!open)} disabled={disabled}>
          <span className={'flex items-center gap-2'}>{String(selectedValue)}</span>
          {dropdownIcon}
        </button>
        {open && (
          <div className="absolute z-10 w-full mt-1">
            {renderList(options, (option: T) => {
              onChange(option)
              toggleOpen(false)
            })}
          </div>
